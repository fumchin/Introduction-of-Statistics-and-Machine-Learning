...
while count < len(species_lines):
    ...
    for col in range(1, len(mt[0])):
        mt[0, col] = col*gap_penalty
    for row in range(1, len(mt)):
        mt[row, 0] = row*gap_penalty
    ...
    for row in range(1, len(mt)):
        for col in range(1, len(mt[0])):
            if species_seq[col-1] == life_x_seq[row-1]:
                ...
                s = mismatch
            mt[row, col] = max(mt[row-1, col-1]+s, mt[row,
                               col-1]+gap_penalty, mt[row-1, col]+gap_penalty)
            # new best
    best_loc = (len(mt)-1, len(mt[0])-1)
    if mt[best_loc[0], best_loc[1]] > best:
        best = mt[best_loc[0], best_loc[1]]
        str1 = ''
        str2 = ''
        update_flag = 1
...
print(best_name + '\n'  + str1 + '\n' + str2)
output = './global_alignment_result.txt'
